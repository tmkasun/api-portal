...
class AuthManager {
  ...
  static getUser(environmentName = Utils.getCurrentEnvironment().label) {
    const userData = localStorage.getItem(
      `${User.CONST.LOCALSTORAGE_USER}_${environmentName}`
    )
    const partialToken = Utils.getCookie(
      User.CONST.WSO2_AM_TOKEN_1,
      environmentName
    )
    const refreshToken = Utils.getCookie(
  ...
  static getUserFromToken() {
    ...
    if (!partialToken) {
      return new Promise((resolve, reject) =>
        reject(new Error("No partial token found"))
      )
    }
  ...
  static hasScopes(resourcePath, resourceMethod) {
    ...
    if (user) {
      ...
      const validScope = APIClient.getScopeForResource(
        resourcePath,
        resourceMethod
      )
  ...
  authenticateUser(username, password, environment) {
    ...
    const data = {
      ...
      scopes:
        "apim:subscribe apim:signup apim:workflow_approve apim:sub_alert_manage",
    }
  ...
  static refresh(environment) {
    const params = {
      refresh_token: AuthManager.getUser(
        environment.label
    ...
    const referrer =
      document.referrer.indexOf("https") !== -1 ? document.referrer : null
